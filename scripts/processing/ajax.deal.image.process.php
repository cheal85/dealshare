...
if($authenticated) {
	...
	//  -------------------------------------------------------------------------------------------------------
	//  Update user avatar
	if($db_manager -> UpdateEntry($post['id_deal'], 'id_image', $post['id'])) {
		$json_array['succes'] = true;
	  	//  -------------------------------------------------------------------------------------------------------
	  	...
	  	//  -------------------------------------------------------------------------------------------------------
	  	//  BUILD MARKUP TO DISPLAY IMAGE
	  	$json_array['image'] = ImageDetails($post['id'], 'medium');
	  	//	-------------------------------------------------------------------------------------------------------
  	}
}
else
{
	$json_array['message'] = 'Image could not be Authenticated';
}
